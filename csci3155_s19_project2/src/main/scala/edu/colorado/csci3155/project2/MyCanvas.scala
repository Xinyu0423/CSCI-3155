...
case class Polygon(val cList: List[(Double, Double)]) extends Figure {
    ...
    override def translate(shiftX: Double, shiftY: Double): Polygon = {
        val n=cList.size;
        var tempList=List[(Double, Double)]();
        for(i<-0 until n) {
            val (x, y) = cList(i);
            val xUpdated = x + shiftX;
            val yUpdated = y + shiftY;
            val newList = (xUpdated, yUpdated);
            tempList = tempList :+ newList;
        }

        return new Polygon(tempList);
    }
...
case class MyCircle(val c: (Double, Double), val r: Double) extends Figure {
    ...
    override def translate(shiftX: Double, shiftY: Double): MyCircle = {
        val (x,y)=c
        val my_circle=MyCircle((x+shiftX,y+shiftY),r)
        return my_circle
...
class MyCanvas (val listOfObjects: List[Figure]) {
    ...
    def getBoundingBox: (Double, Double, Double, Double) = {
        ...
        for(i<-0 until n) {
            val (x1,x2,y1,y2)=listOfObjects(i).getBoundingBox;
            if (xMax < x2) {
                xMax = x2;
            }
    ...
    def translate(shiftX: Double, shiftY: Double): MyCanvas = {
        /*var tempList=List[(Figure)]();
        val n=listOfObjects.size;
        for(i <-listOfObjects){
            tempList:+(i.translate(shiftX,shiftY));
        }
        return new MyCanvas(tempList);*/
    ...
    def placeRight(myc2: MyCanvas):MyCanvas = {
        ...
        //val newList = myc2.listOfObjects.map(f => f.translate(xShift, yShift));
        val c2=myc2.translate(xShift,yShift);
        //new MyCanvas(listOfObjects ++ c2.listOfObjects)
        return this.overlap(c2)
    }
    ...
    def placeTop(myc2: MyCanvas): MyCanvas = {
        val (xmin1,xmax1,ymin1,ymax1)=this.getBoundingBox;
        val (xmin2,xmax2,ymin2,ymax2)=myc2.getBoundingBox;
        val xShift=((xmax1-xmin1)/2-(ymax2-ymin2)/2);
        val yShift=xmax1-xmin1;
        val c2=myc2.translate(xShift,yShift);
        return this.overlap(c2);
    }
    ...
    def rotate(angRad: Double): MyCanvas = {
        var rotateList=List[(Figure)]();
        for(i <-listOfObjects){
            i match {
                case MyCircle(center, r) => {
                    val centerX = center._1
                    val centerY = center._2
                    val newX=centerX*math.cos(angRad)-centerY*math.sin(angRad);
                    val newY=centerX*math.sin(angRad)+centerY*math.cos(angRad);
                    rotateList=rotateList:+new MyCircle((newX,newY),r);
                }
                case Polygon(points) => {
                    var tempList=List[(Double,Double)]();
                    for(p <- points) {
                        val x = p._1
                        val y = p._2
                        val newX=x*math.cos(angRad)-y*math.sin(angRad);
                        val newY=x*math.sin(angRad)+y*math.cos(angRad);
                        tempList=tempList:+(newX,newY);
                    }
                    val newPolygan=new Polygon(tempList);
                    rotateList=rotateList:+newPolygan;
                }
            }


        }
        return new MyCanvas(rotateList);
    }
